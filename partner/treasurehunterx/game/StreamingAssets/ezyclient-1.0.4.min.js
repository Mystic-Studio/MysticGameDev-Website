...
var EzyAppResponseHandler = function() {
    this.handle = function(data) {
        ...
        else EzyLogger.console("app: " + app.name + " has no handler for command: " + cmd)
    }
};
...
var EzyPluginResponseHandler = function() {
    this.handle = function(data) {
        ...
        else EzyLogger.console("plugin: " + plugin.name + " has no handler for command: " + cmd)
    }
};
...
var EzyDisconnectionHandler = function() {
    this.handle = function(event) {
        ...
        var reconnectConfig = config.reconnect;
        var should = this.shouldReconnect(event);
        var mustReconnect = reconnectConfig.enable && reason != EzyDisconnectReason.UNAUTHORIZED && reason != EzyDisconnectReason.CLOSE && should;
        var reconnecting = false;
...
var EzyEventMessageHandler = function(client) {
    ...
    this.handleMessage = function(message) {
        ...
        if (!this.unloggableCommands.includes(cmd)) EzyLogger.console("received cmd: " + cmd.name + ", data: " + JSON.stringify(data));
...
EzyCodecs.getIntBytes = function(value, size) {
    ...
    for (var i = 0; i < size; ++i) {
        var byteValue = value >> (size - i - 1) * 8 & 255;
